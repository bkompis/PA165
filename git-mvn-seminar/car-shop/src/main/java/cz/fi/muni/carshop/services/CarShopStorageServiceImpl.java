...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void addCarToStorage(Car car) {
		if (car.getPrice() < 0){
			throw new IllegalArgumentException("Car price may not be negative.");
		}
	...
	public void sellCar(Car car) throws RequestedCarNotFoundException {
		...
		List<Car> carsOfType = cars.get(car.getType());
		if(carsOfType == null || carsOfType.isEmpty() || !carsOfType.contains(car)){
			throw new RequestedCarNotFoundException("Car not found at delete.");
		}
		carsOfType.remove(car);
	}
}
